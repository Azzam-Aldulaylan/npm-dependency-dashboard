 ...
 * RemoveAnalysisModal (bulk removal, opened from the "Manage dependencies"
 * bulk flow) and the embedded Removal review tab inside
 * ManageDependencyModal (single-package removal, reviewed in place — never
 * a second, separate dialog). No modal chrome of its own.
 */
export function RemoveAnalysisBody({
  packages,
  analysis,
  matchTags,
 ...
 * back in confirm/cancel, same discipline as the upgrade flow. Used only
 * for the bulk-removal flow (opened from the "Manage dependencies" bulk
 * actions modal) — a single-package removal reviewed from Manage dependency
 * renders this same content inline via RemoveAnalysisBody instead, see
 * RemovalReviewPanel.tsx.
 */
export function RemoveAnalysisModal({
  ...
  busy: boolean;
  onConfirm: () => void;
  /** X and Escape always call this — closes the entire flow (releases the host's removal lock). */
  onCancel: () => void;
  onConfigureVerification: () => void;
}): ReactElement {
