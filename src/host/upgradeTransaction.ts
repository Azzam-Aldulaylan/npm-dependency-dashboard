...
export type ExactFileStageFailureCode =
  | 'PATH_NOT_ALLOWLISTED'
  | 'FILE_NOT_FOUND'
  | 'CONFLICT'
  | 'WRITE_FAILED';

export type ExactFileStageResult =
  | { status: 'succeeded'; path: string }
  | {
      status: 'failed';
      path: string;
      code: ExactFileStageFailureCode;
      message: string;
    };
...
export type VerificationResult =
  | {
      status: 'not-run';
      reason: 'not-configured' | 'manifest-stage-failed' | 'file-stage-failed' | 'install-failed' | 'cancelled';
    }
...
export interface UpgradeTransactionResult {
  ...
  /** Results for exact non-manifest files staged by the host, in input order. */
  fileStages?: readonly ExactFileStageResult[];
...
export interface UpgradeTransactionOptions {
  ...
  /**
   * Exact host-generated bytes (normally a reviewed lockfile) to stage before
   * reconciliation. Every path must be a member of allowlistedPaths and must
   * still match expectedContents. package.json is intentionally not inferred
   * or rewritten by this API.
   */
  fileStages?: readonly { path: string; expectedContents: Uint8Array; contents: Uint8Array }[];
...
interface SuccessfulExactFileStage {
  ok: true;
  result: Extract<ExactFileStageResult, { status: 'succeeded' }>;
  expected: ExpectedEntry;
}

interface FailedExactFileStage {
  ok: false;
  result: Extract<ExactFileStageResult, { status: 'failed' }>;
}
...
async function stageExactFile(
  stage: NonNullable<UpgradeTransactionOptions['fileStages']>[number],
  entries: readonly SnapshotEntry[],
  files: UpgradeTransactionFileAdapter
): Promise<SuccessfulExactFileStage | FailedExactFileStage> {
  const snapshot = entries.find((entry) => entry.path === stage.path);
  if (snapshot === undefined) {
    return {
      ...
      result: {
        ...
        path: stage.path,
        code: 'PATH_NOT_ALLOWLISTED',
        message: 'The staged file is not owned by this upgrade transaction.',
      },
    };
  }
  if (!snapshot.before.exists) {
    return {
      ...
      result: {
        ...
        path: stage.path,
        code: 'FILE_NOT_FOUND',
        message: 'The file selected for exact staging no longer exists.',
      },
    };
  }
  ...
  if (!statesEqual(snapshot.before, expectedSource)) {
    return {
      ok: false,
      result: {
        status: 'failed',
    ...
    if (result === 'conflict') {
      return {
        ...
        result: {
          ...
          path: stage.path,
          code: 'CONFLICT',
          message: 'The file changed after the transaction snapshot was captured.',
        },
      };
    }
  } catch (cause) {
  ...
  return {
    ok: true,
    result: { status: 'succeeded', path: stage.path },
    expected: { snapshot, expected: cloneState(staged) },
  };
}
...
export async function runUpgradeTransaction(
  ...
  const notRun: Pick<UpgradeTransactionResult, 'manifestStage' | 'fileStages' | 'install' | 'verification' | 'rollback' | 'retentionDecision'> = {
  ...
  const fileStages: ExactFileStageResult[] = [];
  const expectedAfterStage: ExpectedEntry[] = [];
  if (options.manifestStage !== undefined) {
    ...
    expectedAfterStage.push(...staged.expected);
  }

  for (const fileStage of options.fileStages ?? []) {
    if (cancelled(options.signal)) break;
    const staged = await stageExactFile(fileStage, snapshot.entries, options.files);
    fileStages.push(staged.result);
    if (!staged.ok) {
      const rolledBack =
        expectedAfterStage.length === 0
          ? { status: 'not-needed' as const }
          : await rollback(expectedAfterStage, options.files);
      return {
        completion:
          rolledBack.status === 'not-needed'
            ? 'not-started'
            : completionAfterRollback(rolledBack),
        reason: 'file-stage-failed',
        snapshot: snapshotResult,
        manifestStage,
        fileStages,
        install: { status: 'not-run' },
        verification: { status: 'not-run', reason: 'file-stage-failed' },
        rollback: rolledBack,
        retentionDecision: expectedAfterStage.length === 0 ? 'not-needed' : 'rollback',
      };
    }
    expectedAfterStage.push(staged.expected);

    // Do not continue staging more host-generated files after cancellation.
    // Every already-staged file is still restored only through exact CAS.
    if (cancelled(options.signal)) break;
  }
  ...
  if (cancelled(options.signal)) {
    if (expectedAfterStage.length === 0) {
      return {
        completion: 'not-started',
        reason: 'cancelled',
        snapshot: snapshotResult,
